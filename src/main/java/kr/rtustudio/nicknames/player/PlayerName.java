...
public class PlayerName {
    ...
    public void changeName(CommandSender operator, Player player, String name) {
        ...
        if (name.equals("잘못된닉네임")) {
            notifier.announce(sender, message.get(sender, "nickname.change.unavailable"));
            return;
        }

        switch (validateName(name)) {
            case CONTAINS -> notifier.announce(sender, message.get(sender, "nickname.change.contains"));
            case LENGTH -> notifier.announce(sender, message.get(sender, "nickname.change.length")
                    .replace("{max_length}", String.valueOf(nameConfig.getMaxLength())));
            case DUPLICATED -> notifier.announce(sender, message.get(sender, "nickname.change.duplicated"));
            case SUCCESS -> {
                setName(name);
                String key = operator != null ? "nickname.change.other" : "nickname.change.success";
                notifier.announce(sender, message.get(sender, key)
